...
class Markdown:
	...
	def _setup_extras(self):
		...
		if "header-ids" in self.extras:
			if not hasattr(self, "_count_from_header_id") or self.extras["header-ids"].get(
				"reset-count", False
			):
	...
	def _extract_metadata(self, text: str) -> str:
		...
		def parse_structured_value(value: str) -> Union[list[Any], dict[str, Any]]:
			...
			# Dict
				...
				return {
					match[0].strip(): (match[1].strip() if match[1] else parse_structured_value(match[2]))
				...
				for line in lines[:-1]:  # no var on the last line ("PREFIX End:")
					...
					if continued_for:
						...
						try:
							...
							log.debug(
								"local variables error: missing colon in local variables entry: '%s'" % line
							)
							continue
	...
	def _hash_html_block_sub(self, match: Union[re.Match, str], raw: bool = False) -> str:
	...
	def _strict_tag_block_sub(
		self, text: str, html_tags_re: str, callback: Callable[[str], str], allow_indent: bool = False
	) -> str:
		"""
	...
	def _safe_href(self):
		...
		return re.compile(
			r"^(?:({})?({})({})|(#|\.{{,2}}/)({}))$".format(self._safe_protocols, domain, fragment, fragment),
			re.I,
		)
	...
	@staticmethod
	def _uniform_indent(
		text: str, indent: str, include_empty_lines: bool = False, indent_empty_lines: bool = False
	) -> str:
		"""
	...
	def run(self, text):
		...
		for match in regex_defns.finditer(text):
			...
			replacements.append(
				(
					match.start(0),
					definition_html.format(counter, ref_id, text_before, number, text_after),
					match.end(0),
				)
			)
...
class Tables(Extra):
	...
	def sub(self, match: re.Match) -> str:
		...
		cols = [
			...
			for cell in re.split(split_bar_re, re.sub(trim_bar_re, "", re.sub(trim_space_re, "", underline)))
		]
		align_from_col_idx = {}
		...
		for col_idx, col in enumerate(cols):
			hlines.append(
				"  <th{}>{}</th>".format(align_from_col_idx.get(col_idx, ""), self.md._run_span_gamut(col))
			)
		hlines.append("</tr>")
		hlines.append("</thead>")
		...
		for char in flags_str:
			try:
				...
				raise ValueError(
					...
					"(must be one of '%s')" % (char, s, "".join(list(flag_from_char.keys())))
				)
...
def _html_escape_url(
	attr: str, safe_mode: Union[_safe_mode, bool, None] = False, charset: Optional[str] = None
):
	"""
...
def main(argv=None):
	...
	parser.add_argument(
		"paths", nargs="*", help=("optional list of files to convert.If none are given, stdin will be used")
	)
	parser.add_argument(
		"-v",
		"--verbose",
	...
	parser.add_argument(
		"--html4tags", action="store_true", default=False, help="use HTML 4 style for empty element tags"
	)
	parser.add_argument(
		"-s",
		"--safe",
	...
	parser.add_argument(
		...
		"--extras",
		action="append",
		help="Turn on specific extra features (not part of the core Markdown spec). See above.",
	)
	parser.add_argument(
		"--use-file-vars",
	...
	parser.add_argument("--self-test", action="store_true", help="run internal self-tests (some doctests)")
	parser.add_argument(
		"--compare", action="store_true", help="run against Markdown.pl as well (for testing)"
	)
	parser.add_argument("--output", type=str, help="output to a file instead of stdout")
	parser.set_defaults(
		log_level=logging.INFO, compare=False, encoding="utf-8", safe_mode=None, use_file_vars=False
	)
		...
		try:
			for i, line in enumerate(f.readlines()):
				...
				try:
					...
					raise MarkdownError(
						"%s:%d: invalid link pattern line: %r" % (opts.link_patterns_file, i + 1, line)
					)
	...
	for path in paths:
		...
		if extras and "toc" in extras:
			log.debug(
				"toc_html: " + str(html.toc_html.encode(sys.stdout.encoding or "utf-8", "xmlcharrefreplace"))
			)
		if opts.compare:
