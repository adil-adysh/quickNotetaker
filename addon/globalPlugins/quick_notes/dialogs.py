...
class NoteTakerDialog(wx.Dialog):
	...
	def __init__(self, currentNote=None, noteTitle=None, noteContent=None):
		...
		if self.currentNote and self.currentNote.docxPath:
			...
			checkboxText = _("Update the corresponding Microsoft &Word document also")
		else:
		...
		self.saveAswordCheckBox = sHelper.addItem(wx.CheckBox(self, label=checkboxText))
		if self.currentNote and self.currentNote.docxPath:
	...
	def onCharacter(self, evt):
		...
		if not addonConfig.getValue("autoAlignText"):
			evt.Skip()
			return
		res = handleTextAlignment(content, self.noteEditArea.GetLayoutDirection())
	...
	def saveAsWord(self, newContent):
		...
		if self.currentNote and self.currentNote.docxPath:
			...
			if docxPath is None:
				return
		saveAsWord(
			newContent, docxPath, self._saveAsWordCallback, self.currentNote.id if self.currentNote else None
		)
		self._savePositionInformation()
		self._clean()
	...
	def _saveAsWordCallback(self, outputFilePath, dirWasChanged, mdContent, noteID):
		...
		if notesManagerInstance:
			notesManagerInstance.refreshAllNotesList(notesManagerInstance.notesList.GetFirstSelected())
...
class NotesManagerDialog(
	...
	def __init__(self):
		...
		mainSizer.Add(firstTextSizer, border=guiHelper.BORDER_FOR_DIALOGS, flag=wx.TOP | wx.LEFT | wx.RIGHT)
		...
		# Translaters: The lable of a button in Notes Manager dialog
		openSettingsButton = generalActions.addButton(self, label=_("Open &settings..."))
		openSettingsButton.Bind(wx.EVT_BUTTON, self.onSettings)
		mainSizer.Add(generalActions.sizer, border=guiHelper.BORDER_FOR_DIALOGS, flag=wx.LEFT | wx.RIGHT)

		mainSizer.Add(wx.StaticLine(self), border=guiHelper.BORDER_FOR_DIALOGS, flag=wx.ALL | wx.EXPAND)
	...
	def refreshAllNotesList(self, activeIndex=0):
		...
		for note in notesManager.loadAllNotes():
			self.notesList.Append((note.title, note.lastEdited, getPreviewText(note.content)))
		...
		if allNotesLen > 0:
			...
			self.notesList.SetItemState(activeIndex, wx.LIST_STATE_FOCUSED, wx.LIST_STATE_FOCUSED)
		else:
			self.viewButton.Disable()
			self.editButton.Disable()
			self.copyButton.Disable()
	...
	def onOpenInWord(self, evt):
		curNote = self._getCurrentNote()
		if curNote and curNote.docxPath:
			openInWord(curNote.docxPath, self._openInWordCallback, curNote.id)
			return
		if not curNote:
			return
		docxPath = ""
		if addonConfig.getValue("askWhereToSaveDocx"):
			...
			if docxPath is None:
				return
		saveAsWord(curNote.content, docxPath, self._saveAsWordCallback, curNote.id)
	...
	def _openInWordCallback(self, hasSucceeded, noteID):
		...
		gui.messageBox(
			...
			_(
				"A document with the specified name was not found! You can create a new one so you would be able to view this note as a Microsoft Word document."
			),
	...
	def onCopyAsHtml(self, evt):
		...
		if not curNote:
			return
		res = api.copyToClip(markdown(curNote.content, extras=["markdown-in-html"]), False)
		if res:
			# Translators: the message telling the user that copying the note was successful
			ui.message(_("Copied to clipboard!"))
...
def askUserWhereToSave(parent, noteContent):
	...
	with wx.DirDialog(
		...
		defaultPath=addonConfig.getValue("notesDocumentsPath"),
	) as d:
